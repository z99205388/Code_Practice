...
def index(request):
    ...
    categories = {chart.category for chart in charts}
...
def handle_data_processing(request, file_path, chart_title, description, chart_type, source_type):
    ...
    try:
        ...
        chart = Chart.objects.create(
            ...
            data_file=file_path,
            script_file="data_visualization/views.py (handle_data_processing)",
        )
        ...
        return render(
            request,
            "data_visualization/process_result.html",
            {"success": False, "message": f"Error processing data: {str(e)}", "file_path": file_path},
        )
    ...
    return render(
        request,
        "data_visualization/process_result.html",
        {"chart": chart, "success": True, "message": f'Chart "{chart_title}" has been successfully generated!'},
    )


def parse_file(file_path):
    """文件解析"""
    result = {"data": [], "header": None, "file_type": None, "detal_state": True, "error_message": ""}
    try:
...
def generate_chart(parse_file_result, chart_path, chart_type, chart_title):
    ...
    plotter = {
        ...
        "scatter": lambda: plt.scatter(
            [float(row[1]) for row in data[:50]],
